...
# panel of time series plots of
#  global active power (ul)
#  energy sub metering 1, 2, and 3 (ll)
#  voltage (ur)
#  global reactive power (lr)
...
# four subplots
# (Note: each "plot" function signals a new subplot)
par(mfcol=c(2,2))

# #  global active power (ul)
plot(data3$Date, data3$Global_active_power, pch=".",
     ylab="Global Active Power", xlab="",
     xaxt="n") # xaxt="n" makes xtick and xticklabel invisible
lines(data3$Date, data3$Global_active_power, lty=1)
...
legend("topright", NULL, legend=names(data3[,6:8]),
       lty=c(1,1,1), col=c("black","red","blue"), bty="n") # bty="n" removes legend box
# need to adjust legend font size but leave it for now

# #  voltage (ur)
plot(data3$Date, data3$Voltage, pch=".",
     ylab="Voltage", xlab="datetime",
     xaxt="n") # xaxt="n" makes xtick and xticklabel invisible
lines(data3$Date, data3$Voltage, lty=1)
axis(side=1, at=c(data3$Date[t2[1]],data3$Date[t3[1]],data3$Date[t4[1]]), 
     labels=c(t[t2[1]], t[t3[1]], t[t4[1]]),tick=TRUE)

# #  global reactive power (lr)
plot(data3$Date, data3$Global_reactive_power, pch=".",
     ylab="Global_reactive_power", xlab="datetime",
     xaxt="n") # xaxt="n" makes xtick and xticklabel invisible
lines(data3$Date, data3$Global_reactive_power, lty=1)
axis(side=1, at=c(data3$Date[t2[1]],data3$Date[t3[1]],data3$Date[t4[1]]), 
     labels=c(t[t2[1]], t[t3[1]], t[t4[1]]),tick=TRUE)
